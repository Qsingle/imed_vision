...
from imed_vision.layers import DropPath as TimmDropPath
from imed_vision.comm.helper import _pair as to_2tuple
from imed_vision.comm.init import trunc_normal_
from imed_vision.models.classification import BACKBONE_REGISTER
...
class TinyViT(nn.Module):
    ...
    def get_intermediate_layers(
        ...
        if transpose:
            _outputs = []
            for out in outputs:
                bs, seq_len, ch = out.size()
                res = int(math.sqrt(seq_len))
                o = out.reshape(bs, res, res, ch).permute(0, 3, 1, 2).contiguous()
                _outputs.append(o)
            outputs = _outputs
...
def register_tiny_vit_model(fn):
    ...
    return BACKBONE_REGISTER.register(fn_wrapper)
