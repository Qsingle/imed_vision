...
'''
    @File    :   tree_parse.py
    @Time    :   2023/07/10 09:52:54
    @Author  :   12718 
    @Version :   1.0
    @Desc : this code is from "https://github.com/Puzzled-Hui/ATM-22-Related-Work/blob/main/ATM22-Challenge-Top5-Solution/team_timi/tree_parse.py"
'''
...
def parse(data):
    ...
    nibabel.save(skeleton_nii, os.path.join(sk_root, f))
    # print(ids, f)
    # continue
    skeleton_parse, cd, num = skeleton_parsing(skeleton)
    tree_parsing = tree_parsing_func(skeleton_parse, label, cd)
    ...
    while whether_refinement(parent_map, children_map, tree_parsing, num, trachea) is True:
        ...
        ad_matric = adjacent_map(tree_parsing, num)
        parent_map, children_map, generation = parent_children_map(ad_matric, trachea, num)
    print(ids, '/', length, f, "finished!")
    parse_nii = nibabel.Nifti1Image(tree_parsing, np.eye(4))
    nibabel.save(parse_nii, os.path.join(save_root, f))

def tree_parse_tw(data_root, output_dir, num_worker=4):
    label_root = os.path.join(data_root, "labelsTr")
    save_root = os.path.join(output_dir, "tree_parse_valid")
    sk_root = os.path.join(output_dir, "skeleton")
    if not os.path.exists(sk_root):
        os.makedirs(sk_root)
    if not os.path.exists(save_root):
        os.makedirs(save_root)
    file_list = [os.path.basename(p).split(".")[0] for p in glob.glob(os.path.join(data_root, "imagesTr", "*.nii.gz"))]
    file_list.sort()
    length = len(file_list)
    # threds = [threading.Thread(target=parse, daemon=True) for _ in range(num_worker)]
    # for t in threds:
    #     t.start()
    ...
    #     q.put(data)
    # q.join()
    print("Finished all the parsing, the results save to {}".format(output_dir))

if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser("Tree parse")
    ...
    # q = queue.Queue()
    tree_parse_tw(data_dir, output_dir, num_worker=8)