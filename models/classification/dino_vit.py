...
from comm.init.trunc_norm import trunc_normal_
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:
            x = blk(x)
        x = self.norm(x)
        return {"cls_token": x[:, 0], "img_token": x[:, 1:], "norm_x": x}
...
def dino_vit_tiny(patch_size=16, **kwargs):
