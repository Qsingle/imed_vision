...
class DANet(nn.Module):
    ...
    def forward(self, x):
        ...
        c3 = self.backbone.layer3(net)
        c4 = self.backbone.layer4(c3)
        x = self.head(c4)
        x = list(x)
        x[0] = F.upsample(x[0], size, mode="bilinear", align_corners=True)
        x[1] = F.upsample(x[1], size, mode="bilinear", align_corners=True)
        x[2] = F.upsample(x[2], size, mode="bilinear", align_corners=True)
